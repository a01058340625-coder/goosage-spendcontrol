package com.goosage.api.controller;

import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RestController;

import com.goosage.app.spendcontrol.SpendControlCoachService;
import com.goosage.auth.SessionConst;
import com.goosage.domain.spendcontrol.SpendControlCoachResult;
import com.goosage.support.web.ApiResponse;
import com.goosage.support.web.UnauthorizedException;

import jakarta.servlet.http.HttpSession;

@RestController
public class SpendControlCoachController {

    private final SpendControlCoachService spendControlCoachService;

    public SpendControlCoachController(SpendControlCoachService spendControlCoachService) {
        this.spendControlCoachService = spendControlCoachService;
    }

    @GetMapping("/spend/coach")
    public ApiResponse<SpendControlCoachResult> coach(HttpSession session) {

        Long userId = (Long) session.getAttribute(SessionConst.LOGIN_USER_ID);
        if (userId == null) throw new UnauthorizedException("UNAUTHORIZED");

        System.out.println("[COACH-ENTRY] user=" + userId);

        SpendControlCoachResult result = spendControlCoachService.coach(userId);
        return ApiResponse.ok(result);
    }
}